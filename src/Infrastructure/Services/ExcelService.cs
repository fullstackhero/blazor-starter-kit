...
namespace BlazorHero.CleanArchitecture.Infrastructure.Services
{
    public class ExcelService : IExcelService
    {
        ...
        public async Task<IEnumerable<TEntity>> ImportAsync<TEntity>(Stream stream, Dictionary<string, Func<TEntity, DataRow,object>> mappers, string sheetName = "Sheet1")
        {
            ...
            using (var pck = new ExcelPackage())
            {
                ...
                for (int rowNum = startRow; rowNum <= ws.Dimension.End.Row; rowNum++)
                {
                    ...
                    var x = headers.Select(x => mappers[x](item, row)).ToList();
                    result.Add(item);
