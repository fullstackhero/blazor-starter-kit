...
namespace BlazorHero.CleanArchitecture.Client.Pages.Catalog
{
  public partial class Products
  {
    ...
    private HashSet<GetAllPagedProductsResponse> _selectedItems = new HashSet<GetAllPagedProductsResponse>();
    private GetAllPagedProductsResponse _product = new();
    ...
    {
      var id = this._selectedItems.Select(x => x.Id).ToArray();
      if (id.Any())
      {
        string deleteContent = _localizer["Delete Selected {0} rows"];
        var parameters = new DialogParameters
            {
                {nameof(Shared.Dialogs.DeleteConfirmation.ContentText), string.Format(deleteContent, id.Length)}
            };
        var options = new DialogOptions { CloseButton = true, MaxWidth = MaxWidth.Small, FullWidth = true, DisableBackdropClick = true };
        var dialog = _dialogService.Show<Shared.Dialogs.DeleteConfirmation>(_localizer["Delete"], parameters, options);
        var result = await dialog.Result;
        if (!result.Cancelled)
        {
          var response = await ProductManager.DeleteCheckedAsync(id);
          if (response.Succeeded)
          {
            OnSearch("");
            _snackBar.Add(response.Messages[0], Severity.Success);
